...
<h2  style='color:blue' align='center'>This is Student Details</h2>
<table border="5px black" align="center">
        ...
        @foreach($Students as $t)
            <tr>
                <td><a href="{{route('Student.studentdetails',['student_id'=>$t->student_id,
                    ...
                    'phone'=>$t->phone,'username'=>$t->username,'address'=>$t->address])}}">
                {{$t->name}}</td>
                <td>{{$t->email}}</td>
                <td>{{$t->student_id}}</td>
                <td>{{$t->phone}}</td>
    ...
    <div>
<form action="{{route('Student.ssearch')}}" method="GET">
<input type="text" name="search_stu"
